def build_prompt(journal_sentiment_results, chat_history):
    prompt = f"""
    System Role:
...
def evaluateJournal(daily_journal):
    journal_evaluate_prompt = """
    Your task is to analyze the following daily journals written by a patient and identify 
    the top 3 recurring mood types. 
    ...
    """
    return journal_evaluate_prompt
...
def evaluate_prompt(response):
    ...
    """
    return response_evaluate_prompt

def fine_response(response, evaluation):
    return """
    System Message:
    Based on your evaluation:{evaluation}, I recommend the following course of action:

    - Fine-tune your response: {response} to address the identified areas of improvement.
    - Consider discussing your findings with a mental health professional to ensure they are addressing the root causes of your concerns.
